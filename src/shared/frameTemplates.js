 ...
 */
function getWrapperScriptTemplate(toolCommand, toolName) {
  return `#!/bin/bash
# Frame wrapper script for ${toolName}
...
if ! command -v ${toolCommand} &> /dev/null; then
    echo "Error: ${toolCommand} is not installed or not in PATH"
    exit 1
fi

# Run the tool with initial context prompt
${toolCommand} --prompt "Please read AGENTS.md first and follow the project instructions defined there. This is a Frame-managed project." "$@"
`;
}

/**
 * Generic wrapper template for custom AI tools
 * @param {string} toolCommand - The command to wrap
 * @param {string} toolName - Tool name for comments
 * @param {string} promptFlag - The flag used to pass initial prompt (e.g., '--prompt', '-m')
 */
function getGenericWrapperTemplate(toolCommand, toolName, promptFlag = '--prompt') {
  return `#!/bin/bash
# Frame wrapper script for ${toolName}
# Generated by Frame
...
${toolCommand} ${promptFlag} "Please read AGENTS.md first and follow the project instructions defined there." "$@"
`;
}
